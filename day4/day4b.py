# This file adds very little from the previous one. Could just refactor day4.py to include this 
import os, sys 
import numpy as np
...
def update_boards_state(boards, boards_state, number):
    for i in range(len(boards)):
        ...
        if check_winner(state) and (i not in winner_nos):
            winner_nos.add(i)
            latest_winner.append(caclulate_answer(board, state, number))

for call in bingo_input:
    update_boards_state(boards, boards_state, call)

print(latest_winner[-1])